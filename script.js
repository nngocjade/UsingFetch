...
function previewBoxClick(event) {
  ...
  //     .then(dataObj => {
  //       console.log(dataObj);
  //       return createMessages(dataObj);
  //     }); //saving the json that was return from line 85
  ...
  fetch("http://demo.codingnomads.co:8080/muttsapp/users/3/chats/" + chatID)
    ...
    .then(responseBodyObj => {
      console.log(responseBodyObj);
      let messageArr = responseBodyObj.data;
      messageArr.forEach(messageObj => createChatBubble(messageObj));
    });
}

function createMessages(dataObj) {
